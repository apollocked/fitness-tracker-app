...
class WeightChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (sorted.length < 2) {
      return _card(colors, Center(
        child: Text('Log at least 2 measurements to see the chart',
    ...
    return _card(colors, Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _header(),
            ...
            LineChartBarData(
              ...
              dotData: FlDotData(show: true,
                getDotPainter: (s, p, b, i) => FlDotCirclePainter(
                  radius: 4, color: WeightChartData.dotColor(sorted, i), strokeWidth: 0)),
  ...
  Widget _card(AppColorsExtension c, Widget child) => Container(
    ...
    decoration: BoxDecoration(color: c.cardColor, borderRadius: BorderRadius.circular(16),
      boxShadow: [BoxShadow(color: c.shadowColor, blurRadius: 10, offset: const Offset(0, 3))]),
    child: child,
  );
  ...
  Widget _header() => Padding(
    ...
    child: Row(children: [
      ...
      Text('Weight Trend', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: colors.textColor)),
    ]),
  );

  FlGridData _grid(bool d, double yi) => FlGridData(show: true, drawVerticalLine: false,
  ...
  LineTouchData _touch(List<Measurement> s) => LineTouchData(enabled: true,
    touchTooltipData: LineTouchTooltipData(
      getTooltipItems: (t) => t.map((spot) {
        final i = spot.spotIndex;
        final d = i < s.length ? DateFormat('d MMM').format(s[i].date) : '';
